...
import { Goal, UserRole } from '../types';
import { mockGoals, mockKPIs, mockActionPlans } from '../data/mockData';
import { ArrowLeft, Edit2, Target, TrendingUp, ListTodo, Users, Calendar, AlertCircle } from 'lucide-react';
...
export function GoalDetail({ goalId, userRole, userUnit, onBack, isReadOnly }: GoalDetailProps) {
  const baseGoal = mockGoals.find(g => g.id === goalId);
  const [activeTab, setActiveTab] = useState<'overview' | 'kpis' | 'actions' | 'hierarchy'>('overview');
  const [editableGoal, setEditableGoal] = useState(baseGoal);
  const [isEditing, setIsEditing] = useState(false);

  useEffect(() => {
    setEditableGoal(baseGoal);
    setIsEditing(false);
  }, [baseGoal?.id]);
  ...
  if (!baseGoal) {
    return (
      <div className="bg-white p-12 rounded-lg shadow-sm border border-gray-200 text-center">
        <p className="text-gray-500">Hedef bulunamadı.</p>
  ...
  const goalData = editableGoal ?? baseGoal;
  const relatedKPIs = mockKPIs.filter(k => k.goalId === goalId);
  const relatedActions = mockActionPlans.filter(a => a.goalId === goalId);
  const parentGoal = goalData.parentId ? mockGoals.find(g => g.id === goalData.parentId) : null;
  const childGoals = mockGoals.filter(g => g.parentId === goalId);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div className="flex items-start justify-between mb-4">
          ...
          {(userRole === 'Strategy Office' || goalData.responsibleUnit === userUnit) && !isReadOnly && (
          ...
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-3">
              <span className={`text-xs px-2 py-1 rounded ${
                goalData.level === 0 ? 'bg-purple-100 text-purple-700' :
                goalData.level === 1 ? 'bg-blue-100 text-blue-700' :
                'bg-gray-100 text-gray-700'
              }`}>
                {goalData.level === 0 ? 'Ana Hedef' : goalData.level === 1 ? 'Alt Hedef' : 'Alt Item'}
              </span>
            ...
            <div className="flex flex-wrap gap-2">
              <span className={`px-3 py-1 rounded-full text-sm ${getStatusColor(goalData.status)}`}>
                {goalData.status}
              </span>
              <span className={`px-3 py-1 rounded-full text-sm ${getPriorityColor(goalData.priority)}`}>
                {goalData.priority}
              </span>
            </div>
          </div>
              ...
              {isEditing && (
                <div className="bg-blue-50 border border-blue-100 rounded-lg p-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      <label className="text-sm text-gray-600">Sorumlu Birim</label>
                      <input
                        value={editableGoal?.responsibleUnit ?? ''}
                        onChange={(event) => handleEditChange('responsibleUnit', event.target.value)}
                        className="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2"
                      />
                    </div>
                    ...
                    <div>
                      <label className="text-sm text-gray-600">Baslangic Tarihi</label>
                      <input
                        type="date"
                    ...
                    <div>
                      <label className="text-sm text-gray-600">Bitis Tarihi</label>
                      <input
                        type="date"
                    ...
                    <div className="md:col-span-2">
                      <label className="text-sm text-gray-600">Atanan Kisiler (virgul ile)</label>
                      <input
                  ...
                  <div className="w-full bg-gray-200 rounded-full h-4">
                    <div
                      className={`h-4 rounded-full transition-all ${
                        goalData.status === 'Completed' ? 'bg-green-500' :
                        goalData.status === 'On Track' ? 'bg-blue-500' :
                        goalData.status === 'At Risk' ? 'bg-orange-500' :
                        'bg-red-500'
                      }`}
              ...
              <div className="grid grid-cols-2 gap-6">
                <div className="space-y-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <div className="text-sm text-gray-600 mb-1">Sorumlu Birim</div>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <div>{new Date(goalData.startDate).toLocaleDateString('tr-TR')}</div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <div>{new Date(goalData.endDate).toLocaleDateString('tr-TR')}</div>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <div>{formatAcademicYearRange(goalData.academicYearStart)}</div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-sm">{new Date(goalData.updatedAt).toLocaleString('tr-TR')}</div>
                      <div className="text-xs text-gray-500">Güncelleyen: {goalData.updatedBy}</div>
                    </div>
                  </div>
                </div>
              </div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {goalData.assignedTo.map((person, idx) => (
                      <span key={idx} className="px-4 py-2 bg-blue-50 text-blue-700 rounded-lg">
                        {person}
                      </span>
                    ))}
                  </div>
              ...
              <div className="grid grid-cols-3 gap-4 pt-6 border-t border-gray-200">
                ...
                <div className="bg-green-50 p-4 rounded-lg">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-sm text-gray-600">Aksiyon Planı</span>
                  </div>
          ...
          {activeTab === 'kpis' && (
            <div className="space-y-4">
              {relatedKPIs.length > 0 ? (
                relatedKPIs.map(kpi => (
                  <div key={kpi.id} className="bg-gray-50 p-4 rounded-lg border border-gray-200">
                    <h4 className="mb-2">{kpi.name}</h4>
                    <p className="text-sm text-gray-600 mb-3">{kpi.description}</p>
                    <div className="mb-3">
                      <div className="flex justify-between text-sm mb-1">
                        <span>İlerleme</span>
                        <span>{kpi.currentValue} / {kpi.targetValue} {kpi.unit} ({Math.round((kpi.currentValue / kpi.targetValue) * 100)}%)</span>
                      </div>
                    ...
                    <div className="flex justify-between text-sm text-gray-600">
                      <span>Son Tarih: {new Date(kpi.deadline).toLocaleDateString('tr-TR')}</span>
                      <span>Atanan: {kpi.assignedTo}</span>
                    </div>
                  </div>
                ))
              ) : (
                <p className="text-gray-500 text-center py-8">Bu hedefe bağlı KPI bulunamadı.</p>
              )}
            </div>
          )}
            ...
            <div className="space-y-4">
              {relatedActions.length > 0 ? (
                relatedActions.map(action => (
                  <div key={action.id} className="bg-gray-50 p-4 rounded-lg border border-gray-200">
                    ...
                    <div className="flex justify-between text-sm text-gray-600">
                      ...
                      <span>Son Tarih: {new Date(action.deadline).toLocaleDateString('tr-TR')}</span>
                    </div>
                  </div>
                ))
              ) : (
                <p className="text-gray-500 text-center py-8">Bu hedefe bağlı aksiyon planı bulunamadı.</p>
              )}
            </div>
          )}
          ...
          {activeTab === 'hierarchy' && (
            <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="bg-purple-50 p-4 rounded-lg border border-purple-200">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-sm text-gray-500">{parentGoal.id}</span>
                    </div>
                    <h4>{parentGoal.title}</h4>
                    <p className="text-sm text-gray-600 mt-2">{parentGoal.description}</p>
                  </div>
                </div>
              )}
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    {childGoals.map(child => (
                      <div key={child.id} className="bg-blue-50 p-4 rounded-lg border border-blue-200">
                        ...
                        <h4>{child.title}</h4>
                        <p className="text-sm text-gray-600 mt-2">{child.description}</p>
                        <div className="mt-3">
              ...
              {!parentGoal && childGoals.length === 0 && (
                <p className="text-gray-500 text-center py-8">Bu hedefin hiyerarşik ilişkisi bulunmuyor.</p>
              )}
            </div>
          )}
        </div>
